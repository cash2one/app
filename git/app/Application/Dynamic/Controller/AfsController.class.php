...
class AfsController extends BaseController
{
    ...
    public function downList()
    {
        ...
        foreach ($tagslist as $k => $v) {
            ...
            //$href = C('STATIC_URL') . '/index.php?s=/Dynamic/Afs/tags/tag_name/' . $v['name'];
            ###
            if ($tags == $v['name']) {
                $cur = 'class="cur"';
            } else {
    ...
    //软件标签页
    private function softTag($tag, $type = '')
    {
        ...
        if ($type == 'game') {
            ...
            //分页
            if (!S('total_'.$tag['id'])) {
                $total = M('down')->alias('a')->field('a.id,a.title,a.create_time,a.description,a.smallimg,d.tid,b.size,b.system,c.title cate')->join('__DOWN_DMAIN__ b ON a.id = b.id')->join('__DOWN_CATEGORY__ c ON a.category_id = c.id')->join('__TAGS_MAP__ d ON d.did = a.id', 'left')->where($where)->order('d.sort ASC,a.view DESC')->count();
                S('total_'.$tag['id'], $total, 3600); //缓存总页数,缓存1小时
            } else {
                $total = S('total_'.$tag['id']);
            }
            //分页
            $path = C('STATIC_URL') . '/tag/' . $tag['name'] . '/{page}.html';
            $obj_page = new Page($total, 24, '', false, $path);

            $page_list = $obj_page->show();

            $lists = M('down')->alias('a')->field('a.id,a.title,a.create_time,a.description,a.smallimg,d.tid,b.size,b.system,c.title cate')->join('__DOWN_DMAIN__ b ON a.id = b.id')->join('__DOWN_CATEGORY__ c ON a.category_id = c.id')->join('__TAGS_MAP__ d ON d.did = a.id', 'left')->where($where)->order('d.sort ASC,a.view DESC')->limit($obj_page->firstRow, $obj_page->listRows)->select();
            ...
            //分页
            if (!S('total_'.$tag['id'])) {
                $total = M('down')->alias('a')->field('a.id,a.title,a.create_time,a.description,a.smallimg,b.soft_socre,b.size,b.system,c.title cate')->join('__DOWN_DSOFT__ b ON a.id = b.id')->join('__DOWN_CATEGORY__ c ON a.category_id = c.id')->join('__TAGS_MAP__ d ON d.did = a.id', 'left')->where($where)->order('d.sort ASC,b.soft_socre DESC,a.view DESC')->count();
                S('total_'.$tag['id'], $total, 3600); //缓存总页数,缓存1小时
            } else {
                $total = S('total_'.$tag['id']);
            }
            ...
            $lists = M('down')->alias('a')->field('a.id,a.title,a.create_time,a.description,a.smallimg,d.tid,b.soft_socre,b.size,b.system,c.title cate')->join('__DOWN_DSOFT__ b ON a.id = b.id')->join('__DOWN_CATEGORY__ c ON a.category_id = c.id')->join('__TAGS_MAP__ d ON d.did = a.id', 'left')->where($where)->order('d.sort ASC,b.soft_socre DESC,a.view DESC')->limit($obj_page->firstRow, $obj_page->listRows)->select();
    ...
    //标签页面合集推荐
    protected function unionIntrotion($ids)
    {
        $_tags = array();
        foreach ($ids as $k => $v) {
            $_tags[] = get_tags($ids[$k], 'down');
        }
        ...
        $_tag_ids = array_flip(array_flip($tag_ids));
        $where['a.id'] = array('in',$_tag_ids);
        $result = M('tags')->field('a.id,a.category,a.name,a.title,a.update_time,b.path,a.img')->alias('a')->join('__PICTURE__ b ON a.icon = b.id', 'left')->where($where)->limit(8)->order('a.update_time DESC')->select();
    ...
    {
        $name = I('tag_name') ? I('tag_name') : 'kp';
        $softTag = M('tags')->field('id,name,category,title,meta_title,keywords,description,icon,sub_title')->where('status = 1 AND name = "' . $name . '"')->find();
        //dump($softTag);exit;
        if (!empty($softTag)) {
            if ($softTag['category'] == 1) {
                $this->softTag($softTag, 'game');
            } else {
                $this->softTag($softTag);
            }
            exit();
