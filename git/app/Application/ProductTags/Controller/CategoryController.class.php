...
namespace ProductTags\Controller;
...
class CategoryController extends BaseController{
    ...
    public function index(){
        ...
        $count  = M(ucfirst($model))->alias('a')->join('__PRODUCT_TAGS_MAP__ b on b.did = a.id')->field($fields)->where($where)->count();
        $p = intval(I('p'));
        if (!is_numeric($p) || $p<=0 ) $p = 1;
        if ($p > $count ) $p = $count; //容错处理
        $lists =M(ucfirst($model))->alias('a')->join('__PRODUCT_TAGS_MAP__ b on b.did = a.id')->field($fields)->where($where)->page($p, $row)->select();
        $this->assign('lists',$lists);// 赋值数据集
        $Page       = new \Think\Page($count, $row, '',$tag,false,true,'ProductTags');// 实例化分页类 传入总记录数和每页显示的记录数 和静态分类id
