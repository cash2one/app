...
class P9htWidget extends Controller{
      /**
      * SEO
            ...
            case 'detail':
               $id=empty($id)?'1':$id;
                ...
                if($module=='Package'){
                    //礼包的规则 
                    //1、seotitle
                    //2、礼包名字_礼包名字领取_9HT礼包中心
                    if(!empty($detail['seo_title'])){
                        $title = $detail['seo_title']; 
                    }else{
                        $title = $detail['title'] . '_' . $detail['title'] .'领取_9HT礼包中心'; 
                    }
                }else{
                    //其他的规则 
                    //1、seo title
                ...
                if(empty($detail['seo_description'])){
                    if ($module=='Package') {
                        // 礼包
                        $seo['description'] = "{$detail['title']}的领取,{$detail['title']}使用说明,{$detail['title']}使用说明注册激活的帮助由9HT礼包中心免费为您提供";
                    }else{
                        // 其他
