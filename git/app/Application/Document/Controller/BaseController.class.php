...
class BaseController extends Controller {
     ...
     * @return void
     */
    protected function SEO($params = array()){
        $title = $params['title'] ? $params['title'] : C('PACKAGE_DEFAULT_TITLE');
        $this->assign('seo_title', $title);

        $key = $params['key'] ? $params['key'] : C('PACKAGE_DEFAULT_KEY');        
        $this->assign('seo_key', $key);

        $description = $params['description'] ? $params['description'] : C('PACKAGE_DEFAULT_DESCRIPTION');
