...
namespace I\Api;
use I\Api\Api;
use I\Model\UcenterMemberModel;
...
class UserApi extends Api{
   ...
   /**
     ...
     * @author 刘盼
     */
    public function updateInfo($uid, $password, $data,$encrypted){
        if($this->model->updateUserFields($uid, $password, $data,$encrypted) !== false){
            $return['status'] = true;
        }else{
            $return['status'] = false;
     ...
     * @author 刘盼
     */
    public function updateUserInfo($uid, $data,$encrypted){
        if($this->model->updateUserData($uid,$data,$encrypted) !== false){
            $return['status'] = true;
        }else{
            $return['status'] = false;
            $return['info'] = $this->model->getError();
    ...
    public function updateUserToken($username,$password,$data){
        if($this->model->updateUserToken($username,$password,$data) !== false){
            $return['status'] = true;
        }else{
            $return['status'] = false;
            $return['info'] = $this->model->getError();
