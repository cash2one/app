...
class AttributeModel extends Model
{
    ...
    protected $_validate = array(
        ...
        array('name', 'checkName', '在本表或者主表中字段名已存在', self::MUST_VALIDATE, 'callback', self::MODEL_BOTH),
    ...
    protected function checkName()
    {
        ...
        // 主表和附属表都不能有同一个字段
        $map['name'] = $name;
        $map['model_id'] = $extend ? array('in', array($model_id, $extend)) : $model_id;
        if (!empty($id)) {
            $map['id'] = array('neq', $id);
        }
    ...
    protected function checkTableExist($model_id)
    {
        ...
        if ($model['extend'] == 0) {  //独立模型表名
            ...
            $table_name = $this->table_name = C('DB_PREFIX').strtolower($extend_model['name']).
