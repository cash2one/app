 ...
 * 后台标签管理控制器
 */
class ProductTagsController extends AdminController {
    ...
    */
    public function index($category){
        ...
        //统计排序 add liujun 2015-07-22
        $column = 'id,id as tagid,name,title,sort,pid,status,category,(select count(*) from '.C(DB_PREFIX).'product_tags_map where tid = tagid and did > 0) as total';
        $order = 'update_time desc,sort desc';//默认排序
        ...
        if(!empty($field) && !empty($keyword)){
            ...
            $tree = M('ProductTags')->field($column)->where($map)->order($order)->select(); //用于搜索，但是没有保持树形结构
        }
        else
        {
            $tree = D('ProductTags')->getTree(0, $column, $map,$order);
        }
        ...
        foreach($tree as $key=>$value){
        	...
        	foreach($mTypes as $m_key=>$m_value){
        		$mCount = M('ProductTagsMap')->field('count(*) as count')->where(array('type'=>$m_key,'tid'=>$value['id']))->find();
        ...
        C('_SYS_GET_PRODUCT_TAGS_TREE_', true); //标记系统获取标签树模板
    ...
    public function tree($tree = null){
        C('_SYS_GET_PRODUCT_TAGS_TREE_') || $this->_empty();
        $this->assign('tree', $tree);
        $this->display('tree');
    }
    ...
    public function edit($id = null, $pid = 0){
        ...
        if(IS_POST){ //提交表单
            if(false !== $Tags->update()){
                $this->success('编辑成功！',U('ProductTags/index/category/'.I('category')));
            } else {
                $error = $Tags->getError();
    ...
    public function add($category , $pid = 0){
        ...
        if(IS_POST){ //提交表单
            if(false !== $Tags->update()){
                $this->success('新增成功！',U('ProductTags/index/category/'.I('category')));
            } else {
                $error = $Tags->getError();
                $this->error(empty($error) ? '未知错误！' : $error);
            }
    ...
    public function remove(){
        ...
        foreach ($ids as $id) {
            ...
            $child = M('ProductTags')->where(array('pid'=>$id))->field('id')->select();
            if(!empty($child)){
        ...
        //删除该标签信息
        $res = M('ProductTags')->delete(implode(',', $ids));

        if($res !== false){
            foreach ($ids as $id) {
                //记录标签删除行为
                action_log('delete_product_tags', 'productTags', $id, UID);
            }
            //删除该标签映射信息
            $res_map= M('ProductTagsMap')->where(array('tid'=>array('in',$ids)))->delete();
            if($res_map !== false){
                foreach ($ids as $id) {
                    //记录标签映射删除行为
                    action_log('delete_product_tags_map', 'productTags', $id, UID);              
                }
                $this->success('删除标签成功！');
            }
        }else{
    ...
    public function operate($type = 'move'){
        ...
        $list = M('ProductTags')->where($map)->field('id,pid,title')->select();
        ...
        if(strcmp($type, 'move') == 0){
        	...
        	$pid = M('ProductTags')->getFieldById($from, 'pid');
    ...
    public function move(){
        ...
        $cate_id =  M('ProductTags')->field('category')->where(array('id'=>$id))->find();
        if(is_numeric($cate_id['category'])){
            $data['pid'] = $to;
            $data['category'] = $cate_id['category'];
            $data['id'] = $from;
            $res = M('ProductTags')->save($data);
            if($res !== false){
