...
class AdminController extends Controller {
    ...
    final protected function editRow ( $model ,$data, $where , $msg ){
        ...
        if( M($model)->where($where)->save($data)!==false ) {
            //crohn 2015_01_07 新增状态修改日志
            action_log('setstatus_'.$data['status'], $model, $id, UID);
    ...
    public function setStatus($Model=CONTROLLER_NAME){
        ...
        if(is_array($ids) && !empty($ids))
        {
            ...
            foreach($ids as $id)
            {
                $model->createStaticDetail($id);
                $model->createStaticDetailM($id);
            }

        }

        //如果存在子文章，则不允许直接操作 sunjianhua 2016.2.2
        if ($Model === 'Document') {
            $exclude_ids = array();
            if (!is_array($ids)) {
                $ids = array($ids);
            ...
            if ($exclude_ids) {
                ...
                //存在子文档，不能直接操作
                $this->error('操作失败。被操作文档[编号：'. $exclude_ids .']存在子数据，请先操作文档的子数据', '', IS_AJAX);
                exit();
            }
        }
