...
class AttributeController extends AdminController {
    ...
    public function add(){
        $model_id   =   I('get.model_id');
        $model      =   M('Model')->field('title,name,field_group,extend')->find($model_id);
        $this->assign('model',$model);
        $this->assign('info', array('model_id'=>$model_id, 'extend'=>$model['extend']));
     ...
     */
    public function copyBaseDocument(){
        return;
        $model_id   =   I('get.model_id');
        $my_list = D('Attribute')->where('model_id='. $model_id)->select();
        //获取主模型ID
        $mid = get_model_by('name', 'document', 'id');
        $d_list = D('Attribute')->where('model_id='.$mid['id'])->select();
        ...
        foreach ($my_list as $key => $value) {
            foreach ($d_list as $k => $v) {
                if ($value['name']==$v['name']){
                    ...
                    var_dump($value['id'], $data);
                    D('Attribute')->where('id='. $value['id'])->save($data);
                }
            }
        }
