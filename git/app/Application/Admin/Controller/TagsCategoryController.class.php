 ...
 * 后台标签分类管理控制器
 */
class TagsCategoryController extends AdminController {
    ...
    public function remove(){
        ...
        //判断该分类下有无标签，有则不允许删除
        $tags =  M('Tags')->where('category='.$id)->field('id')->select();
        if(!empty($tags)){
            $this->error('请先删除该分类下的标签');
        }

        //删除该标签信息
        $res = M('TagsCategory')->delete($id);

        if($res !== false){
            //记录标签删除行为
            action_log('delete_tags_categpry', 'tagsCategory', $id, UID);      
