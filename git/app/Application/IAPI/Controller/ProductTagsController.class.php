...
class ProductTagsController extends DataBaseController{
    /**
     * 描述:不传参数则获取所有的标签
    ...
    public function get(){
        ...
        if($model_id){
            //有则返回缓存
            $result     =   (!$tree?S('IAPI_product_tags'.$model_id):S('IAPI_product_tags_tree'.$model_id));
            if($result) $this->ajaxReturn($result,$this->returnType,false);

            //根据模型id 获取绑定标签分类
            $tagCate    =   M('Model')->where('id = '.$model_id)->getField('product_tags_category');
            if(empty($tagCate))
            ...
            //根据模型id 获取绑定标签分类
            $tagCate    =   M('ProductTagsCategory')->where('status = 1')->getField('id',true);
            if(empty($tagCate))
                $this->ajaxReturn(null,$this->returnType);
        ...
        $tags['tags']       =   M('ProductTags')->field('id,category,name,title,pid')->where($where)->order('id DESC')->select();

        //数据缓存 并输出
        if(!$tree){
            $arr        =   json_encode($tags);
            S('IAPI_product_tags'.$model_id,$arr,1200);
    ...
    //        $arr_tree   =   list_to_tree($tags,'id','pid','child',0);
            ...
            S('IAPI_product_tags_tree'.$model_id,$arr_tree,1200);
